    ...
    int shortestPathAllKeys(vector<string>& grid) 
    {
          ...
          for(int i = 0 ; i < grid.size() ; i++){
             for(int j = 0 ; j < grid[i].length() ; j++){
                 ...
                 else if(grid[i][j] >= 'a' and grid[i][j] <= 'z'){ //check
                    keys++;
                 }
             }
          }
                   ...
                   if(isValid(row+r,col+c) == true)
                   {
                        if(grid[row+r][col+c] == '.'){
                             if(vis[row+r][col+c][mask] == false){
                                  ...
                                  q.push({row+r,col+c,mask,steps+1});    //with the curr mask we are trying to get on the cell row+r , col+c
                             }
                        }
                             ...
                             if(vis[row+r][col+c][mask] == false){
                                  ...
                                  q.push({row+r,col+c,updated_mask,steps+1});    //with the curr mask we are trying to get on the cell row+r , col+c
                             }
                        }
                        else{
                             if(vis[row+r][col+c][mask] == false){
                                 ...
                                 //checking if in the curr mask we have the key for this lock or not
                                 int found = (mask & (1 << (key - 'a'))); 
                                 if(found > 0) q.push({row+r,col+c,mask,steps+1});   //finally we can land on the cell row+r col+c after unlocking it
